...
public interface DevServicesBuildTimeConfig {
     ...
     * For all services but Cognito, the local AWS stack will be provided by MiniStack.
     * Otherwise, it will be provided by Moto
     */
    Optional<Boolean> enabled();

    /**
     * Indicates if the MiniStack container managed by Dev Services is shared.
     * When shared, Quarkus looks for running containers using label-based service discovery.
     * If a matching container is found, it is used, and so a second one is not started.
     * Otherwise, Dev Services starts a new container.
     * <p>
     * The discovery uses the {@code quarkus-dev-service-ministack} label.
    ...
    /**
     * Indicates if shared MiniStack services managed by Dev Services should be isolated.
     * When true, the service will be started in its own container and the value of
     * the {@code quarkus-dev-service-ministack} label will be suffixed by the service name (s3, sqs, ...)
     */
     ...
     * <p>
     * This property is used when you need multiple shared MiniStack instances.
     */
    @WithDefault(value = "ministack")
    String serviceName();
