...
If you're going to use a local {service-name-uppercase} instance (for example a MiniStack), configure it as follows:
...
----
<1> Override the {service-name-uppercase} client to use MiniStack instead of the actual AWS service
<2> MiniStack defaults to `us-east-1`
