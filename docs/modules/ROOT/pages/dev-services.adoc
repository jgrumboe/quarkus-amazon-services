...
Quarkus Amazon Services automatically starts a MiniStack container in dev mode and when running tests. So, you don't have to start one manually. The extension client is configured automatically.
...
By default, Dev Services for Amazon Services will start one MiniStack container with only the needed services.
...
Dev Services for Amazon Services uses `ministackorg/ministack` image. You can configure the image and version using the `quarkus.aws.devservices.ministack.image-name` property:

[source,properties]
----
quarkus.aws.devservices.ministack.image-name=ministackorg/ministack:1.2
----
...
[source,properties]
----
quarkus.aws.devservices.ministack.container-properties.START_WEB=0
...
The key is the name of the service to enable and must be a valid MiniStack service name.
...
The Cognito extension is not using MiniStack but Moto.
